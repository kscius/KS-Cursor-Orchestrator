...
- **Cursor CLI first (`/agent-dispatch`):** When the TASK is explicitly **batch via CLI / headless `agent -p`**, or the agreed plan / scout artifact set **`recommended_cli: yes`**, run **`node …/hooks/agent-dispatch.js`** (see **`commands/agent-dispatch.md`** and **`commands/cli-batch.md`**) **before** heavy in-IDE edits; then continue in-IDE only for remaining work, validation, and review. Prefer **`--mode ask`** or **`plan`** unless the user asked for mutating batch (**`--force`**). Follow **`commands/ks-conductor.md`** → **Cursor CLI (condicional)** for when **not** to use the CLI.
